...
export default function ServerLicenseSlide({
  ...
  const overLimitBody = (
    <Trans
      ...
      defaults="Our licensing permits up to <strong>{{freeTierLimit}}</strong> users for free per server. You have <strong>{{overLimitUserCopy}}</strong> My PDF users. To continue uninterrupted, upgrade to the My PDF Server plan - <strong>unlimited seats</strong>, PDF text editing, and full admin control for $99/server/mo."
    />
  );
  ...
  const freeBody = (
    <Trans
      ...
      defaults="Our <strong>Open-Core</strong> licensing permits up to <strong>{{freeTierLimit}}</strong> users for free per server. To scale uninterrupted, we recommend the My PDF Server plan - <strong>unlimited seats</strong> and <strong>SSO support</strong> for $99/server/mo."
    />
  );
